...
internal static class PortableTextureImageProviderProfileProtector
{
    private const string Prefix = "huttextureimage:v1:";
    private const string KeyMaterial = "HUnityAutoTranslator.TextureImageProviderProfiles.Portable.v1";

    public static string Protect(string value)
    {
        if (string.IsNullOrEmpty(value))
        {
            throw new ArgumentException("Texture image provider profile content must not be empty.", nameof(value));
        }
    ...
    public static string Unprotect(string protectedValue)
    {
        ...
        {
            throw new CryptographicException("Texture image provider profile payload is not recognized.");
        }

        var payload = Convert.FromBase64String(protectedValue.Substring(Prefix.Length));
        if (payload.Length <= 48)
        {
            throw new CryptographicException("Texture image provider profile payload is too short.");
        }
        ...
        {
            throw new CryptographicException("Texture image provider profile payload authentication failed.");
        }

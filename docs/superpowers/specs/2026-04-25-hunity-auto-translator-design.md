...
- `TranslationQueue`：后台批处理、多并发翻译、限流、重试和结果回传。
- `ResultDispatcher`：接收翻译结果，并在 Unity 主线程尽快写回游戏 UI。
...
- 短文本可批量翻译。
- 长文本单独翻译。
- 使用可配置 worker pool 支持多线程、多并发翻译。
- 并发数、每分钟请求数、批量最大字符数可配置，并受 provider 独立限流器约束。
- 不同批次可以并发请求，同一原文只保留一个在途请求，避免重复消耗额度。
- 队列过长时合并重复项。
- 队列满时优先保留当前可见、较新的、非重复文本。
- 当前可见文本和短 UI 文本拥有更高优先级，优先进入下一批请求。
- 翻译失败后指数退避重试。
- 最终失败则保留原文并记录错误。

### 4.3 结果回写与延迟控制

翻译结果返回后不等待下一轮扫描，而是立即进入 `ResultDispatcher`。`ResultDispatcher` 在 Unity `Update` 中从线程安全队列取出结果，并在主线程写回对应文本组件。

延迟控制策略：

- 后台 worker 完成翻译后立即发布结果。
- 主线程下一帧优先处理新完成结果。
- 当前可见组件优先写回。
- 每帧写回数量有预算，避免一次性改太多 UI 导致掉帧。
- 若预算不足，结果保留在高优先级写回队列中，后续帧继续应用。
- 如果组件已销毁、文本已被游戏改成新原文，跳过旧结果，避免覆盖游戏新内容。
...
- UGUI/TMP 扫描低频运行。
- 每轮扫描有对象数量预算。
- 每帧写回有数量预算，但新完成结果优先写回，缩短翻译完成到游戏内更新的延迟。
- 后台翻译使用多线程 worker pool 和异步 HTTP 请求。
- 后台翻译有并发限制和 provider 独立限流，避免过量请求导致限速或卡顿。
- 日志限频，避免刷屏。
- 缓存命中立即返回，不进入网络队列。
...
- 控制面板能实时修改后台翻译并发数、每分钟请求上限、批量字符上限。
- UGUI、TMP、IMGUI 至少各有一条验证路径。
- 翻译请求不会阻塞 UI。
- 多个翻译批次可以并发执行，并受限流器控制。
- 翻译完成后能在主线程下一帧优先写回，缩短显示延迟。
- 缓存命中后可快速显示译文。
...
- 后台翻译必须支持可配置多线程、多并发，以加快首次翻译速度。
- 翻译完成后通过主线程结果分发器尽快写回游戏 UI，避免等待下一轮扫描。
